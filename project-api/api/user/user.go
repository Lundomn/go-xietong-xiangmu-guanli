...
func (*HandlerUser) getCaptcha(ctx *gin.Context) {
	...
	if rsp == nil {
		ctx.JSON(http.StatusOK, result.Fail(http.StatusBadGateway, "用户服务返回为空"))
		return
	}
	if os.Getenv("MS_CAPTCHA_EXPOSE_CODE") == "1" {
		// Development-only compatibility for the demo flow. In production the
		// code should be delivered by an SMS provider, never returned by HTTP.
		ctx.JSON(http.StatusOK, result.Success(rsp.Code))
		return
	}
	ctx.JSON(http.StatusOK, result.Success(gin.H{"sent": true}))
}
...
func (u *HandlerUser) login(c *gin.Context) {
	...
	if loginRsp == nil {
		c.JSON(http.StatusOK, result.Fail(http.StatusBadGateway, "用户服务返回为空"))
		return
	}
	rsp := &user.LoginRsp{}
...
func (u *HandlerUser) myOrgList(c *gin.Context) {
	...
	ctx, cancel := context.WithTimeout(c.Request.Context(), 2*time.Second)
	defer cancel()
	list, err2 := rpc.LoginServiceClient.MyOrgList(ctx, &login.UserMessage{MemId: memberId})
	if err2 != nil {
