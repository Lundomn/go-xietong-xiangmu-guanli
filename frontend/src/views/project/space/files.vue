        ...
        <wrapper-content :showHeader="false">
            <div class="content-wrapper">
                <div class="content-item log-list">
                    <div class="header">
                        <span class="title">{{deletedFiles ? '文件回收站' : '项目文件'}}</span>
                        <div class="header-actions">
                            <!--<a><a-icon type="upload"></a-icon> 上传文件</a>-->
                            <a-button v-if="!deletedFiles" id="upload-file" icon="up-circle" type="dashed">上传</a-button>
                            <a-button class="m-l-sm" icon="delete" @click="toggleDeletedFiles">
                                {{deletedFiles ? '返回文件' : '文件回收站'}}
                            </a-button>
                        </div>
                    </div>
                    <div class="list-content">
                        ...
                        >
                            ...
                            <a-list-item class="list-item" :key="index" v-for="(item, index) in files">
                                <a-list-item-meta>
                                    ...
                                    <div slot="title">
                                        <a-tooltip :mouseEnterDelay="0.3">
                                            ...
                                            <a class="text-default" href="javascript:void(0)" @click="downloadFile(item)"
                                ...
                                <span slot="actions">
                                    <a-tooltip title="下载">
                                        <a class="muted" href="javascript:void(0)" @click="downloadFile(item)">
                                ...
                                <span slot="actions" v-if="!deletedFiles" @click="editFile(item,index)">
                                    <a-tooltip title="编辑">
                                        <a-icon type="edit"/>
                                    </a-tooltip>
                                </span>
                                <a class="muted" slot="actions">
                                    <a-dropdown :trigger="['click']" placement="bottomCenter">
                                        ...
                                        <a-menu class="field-right-menu"
                                            ...
                                            <a-menu-item v-if="!deletedFiles" key="delete">
                                                <a-icon type="delete"/>
                                                <span>移到回收站</span>
                                            </a-menu-item>
                                            <a-menu-item v-if="deletedFiles" key="recovery">
                                                <a-icon type="undo"/>
                                                <span>恢复文件</span>
                                            </a-menu-item>
                                            <a-menu-item v-if="deletedFiles" key="permanentDelete">
                                                <a-icon type="delete"/>
                                                <span>彻底删除</span>
                                            </a-menu-item>
                                        </a-menu>
...
<script>
    ...
    import {checkResponse, downloadAuthenticated} from "../../../assets/js/utils";
    import {relativelyTime} from "../../../assets/js/dateTime";
    import {del, edit, list, recycle, recovery} from "../../../api/file";
    ...
    export default {
        ...
        methods: {
            ...
            getFiles(reset = true) {
                ...
                app.requestData.deleted = this.deletedFiles ? 1 : 0;
            ...
            toggleDeletedFiles() {
                this.deletedFiles = !this.deletedFiles;
                this.getFiles();
            },
            downloadFile(file) {
                downloadAuthenticated(file.file_url, file.fullName || file.title || '下载文件')
                    .catch(error => this.$message.error(error.message || '文件下载失败'));
            },
            collectProject() {
            ...
            editFile(file, index) {
                ...
                this.$nextTick(() => {
                    const input = app.$refs[`inputTitle${index}`];
            ...
            onCellChange(file) {
                ...
                const suffix = extension && extension.charAt(0) === '.' ? extension : (extension ? '.' + extension : '');
                const fullName = suffix && !title.toLowerCase().endsWith(suffix.toLowerCase()) ? title + suffix : title;
                if (!fullName) {
                    currentFile.title = currentFile.fullName;
                    return;
                }
                if (fullName != currentFile.fullName) {
                    edit({title: fullName, fileCode: currentFile.code}).then(res => {
                        const result = checkResponse(res);
                        if (!result) {
                            return false;
                        }
                        currentFile.title = fullName;
            ...
            doFile(action, fileCode) {
                ...
                switch (actionKey) {
                    ...
                    case 'permanentDelete':
                        this.$confirm({
                            title: '彻底删除文件',
                            content: '彻底删除后将无法恢复，确定继续吗？',
                            okText: '彻底删除',
                            okType: 'danger',
                            cancelText: '取消',
                            onOk() {
                                return del(fileCode).then((res) => {
                                    if (!checkResponse(res)) {
                                        return false;
                                    }
                                    app.getFiles();
