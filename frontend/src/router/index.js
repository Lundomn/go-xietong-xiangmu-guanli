...
const appendMenuRoutes = (items, parentItems) => {
    ...
    items.forEach(function (item) {
        ...
        if (route) {
            parentItems.push(route);
        }
        if (item && item.children) {
            appendMenuRoutes(item.children, parentItems);
        }
    });
};
if (menu) {
    appendMenuRoutes(menu, routes);
}
const router = new Router({
    routes: [
        {
...
router.beforeEach((to, from, next) => {
    ...
    //无效页面跳转至首页
    if (!to.name && (!from.meta || from.meta.model !== 'Login') && to.path !== HOME_PAGE) {
        next({path: '/404'});
        return false;
    }
    if (to.meta && to.meta.model === 'Login' && store.state.logged) {
        next({path: HOME_PAGE});
        return false;
    }
    if (!store.state.logged && (!to.meta || (to.meta.model !== 'Login' && to.meta.model !== 'error'))) {
        next({
            name: 'login',
