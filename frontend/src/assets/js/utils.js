...
export const createRoute = (data) => {
    ...
    let path = '/' + String(data.url).replace(/^\/+/, '');
    if (data.params) {
        path += '/' + String(data.params).replace(/^\/+/, '');
    }
    let filePath = data.file_path || data.url;
...
export const getApiUrl = (api) => {
    const base = (process.env.VUE_APP_API_URL || '').replace(/\/+$/, '');
    const path = String(api || '').replace(/^\/+/, '');
    const crossDomain = ['true', '1'].indexOf(String(process.env.VUE_APP_CROSS_DOMAIN || '').toLowerCase()) !== -1;
    if (base && (crossDomain || process.env.NODE_ENV === 'production')) {
        return base + '/' + path;
    }
    if (process.env.NODE_ENV === 'production') {
        // Keep API traffic under a dedicated prefix so `/project/...` can
        // remain available to Vue Router as a client-side route.
        return '/api/' + path;
    }
    return '/api/' + path;
};
...
export const downloadAuthenticated = (url, filename = '下载文件') => {
    return Axios.get(url, {
        ...
        if (contentType && contentType.indexOf('application/json') !== -1) {
            return response.data.text().then(text => {
                ...
                throw new Error(payload.msg || '文件下载失败');
            });
        }
...
//   return arrs;
// }
export const snail = (value, obj) => {
    if (obj === null || obj === undefined) {
        return false;
    }
    if (Array.isArray(obj)) {
        return obj.some(item => snail(value, item));
    }
    ...
    return Object.keys(obj).some(key => {
        return obj[key] && typeof obj[key] === 'object' && snail(value, obj[key]);
    });
};
