...
func (p *ProjectDao) UpdateProject(ctx context.Context, proj *data.Project) error {
	return p.conn.Session(ctx).Model(&data.Project{}).
		Where("id=?", proj.Id).
		Select("name", "description", "cover", "task_board_theme", "prefix", "private", "open_prefix", "open_begin_time", "open_task_private", "schedule", "auto_update_schedule").
		Updates(proj).Error
}
...
func (p ProjectDao) FindCollectProjectByMemId(ctx context.Context, memberId int64, page int64, size int64) ([]*data.ProjectAndMember, int64, error) {
	...
	session := p.conn.Session(ctx)
	index := (page - 1) * size
	sql := fmt.Sprintf("select * from ms_project where deleted=0 and id in (select project_code from ms_project_collection where member_code=?) order by sort limit ?,?")
	raw := session.Raw(sql, memberId, index, size)
	if err := raw.Scan(&pms).Error; err != nil {
		return nil, 0, err
	}
	var total int64
	query := "select count(*) from ms_project_collection c join ms_project p on p.id=c.project_code where c.member_code=? and p.deleted=0"
	err := session.Raw(query, memberId).Scan(&total).Error
