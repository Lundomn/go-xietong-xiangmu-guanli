...
func (c *CacheInterceptor) Cache() grpc.ServerOption {
	return grpc.UnaryInterceptor(func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp interface{}, err error) {
		...
		if respJson != "" {
			cached := reflect.New(reflect.TypeOf(respType).Elem()).Interface()
			if err := json.Unmarshal([]byte(respJson), cached); err == nil {
		...
		if err == nil && resp != nil {
			...
			if marshalErr == nil {
				_ = c.cache.Put(con, info.FullMethod+"::"+cacheKey, string(bytes), 5*time.Minute)
